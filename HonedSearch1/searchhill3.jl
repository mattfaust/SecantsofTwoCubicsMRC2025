...
"""
function count_real_pairs(sols::Vector{Vector{ComplexF64}})
    count(s -> isreal(s[1]) && isreal(s[2]) &&
               isreal(s[5]) && isreal(s[6]),
          sols)
end
...
"""
function count_mixed_pairs(sols::Vector{Vector{ComplexF64}})
    count(s -> (
        # real pair on (z1,z2) and conj pair on (z5,z6)
        isreal(s[1]) && isreal(s[2]) && isapprox(s[5], conj(s[6]), atol=tol) && !isreal(s[5]) ||
        # or conj pair on (z1,z2) and real pair on (z5,z6)
        isapprox(s[1], conj(s[2]), atol=tol) && !isreal(s[1]) && isreal(s[5]) && isreal(s[6])
    ), sols)
end
