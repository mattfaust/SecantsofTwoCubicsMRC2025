...
# 5) Hill‑climb loop
# ---------------------------------------------------
target_m   = 10             # goal: 10 conjugate‑pairs
radius     = 0.1
max_iter   = 2

# your initial parameter vector p0_vec must match the seeds
p_prev_vec = [0.18618657992246557, 1.0708403923495844, 0.10325619205528191, 7.164509434808381, 0.808048339635927, 8.562191790780727, 1.6013191365536088, 14.721850583330937, 5.268090443236682, 0.5012381267755297, 2.6460958485858512, 8.620126073982671]
p_prev     = Dict(zip(params, p_prev_vec))

best_count    = count_conj(sol_prev)
best_residual = total_residual(sol_prev)

println("Starting count = $best_count, residual = $best_residual")

for iter in 1:max_iter
    # propose new p
    Δ         = radius .* (2 .* rand(12) .- 1)
    p_new_vec = p_prev_vec .+ Δ
    p_new     = Dict(zip(params, p_new_vec))

    # track only your 10 starts via parameter homotopy
    result = solve(F, sol_prev; parameters=params, start_parameters=p_prev,target_parameters=p_new,show_progress = false)
    ...
    if (c > best_count) || (c == best_count && r < best_residual)
        println("  ↳ accepted (was count=$best_count, resid=$(round(best_residual,digits=20)))")
        global best_count    = c
        global best_residual = r
        global sol_prev      = sols
        global p_prev_vec    = p_new_vec
        global p_prev        = p_new
    else
        println("  ↳ rejected")
    end

    if best_count ≥ target_m
