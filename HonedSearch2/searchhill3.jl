...
#curres = solve(
#  sys;
#  start_system       = :polyhedral,  # (this is the default)
#  target_parameters  = p0_vec,
#  show_progress      = true
#)
#sol_prev = solutions(curres)
...
is_near_real(z::ComplexF64) = abs(imag(z)) < tol_near_real
is_near_complex_pair(z1::ComplexF64,z2::ComplexF64) = norm(z1-conj(z2)) < tol
...
p0_vec =[0.016083851925985188, 0.81992084645368, 0.18288704715031662, 7.217155365291252, 0.5765830430706698, 8.317898201682171, 1.1850395896226666, 14.16847320137364, 5.538481931313884, 0.938535690257467, 3.325990675468095, 8.292236451552156]
p_prev     = Dict(zip(sys.parameters, p0_vec))


best_c    = count_conj(sol_prev) + count_mixed_pairs(sol_prev)
...
#	print(failed(result))
    ...
    if (cr > best_c) || (cr >= best_c && r < best_r && r_real == 0)
